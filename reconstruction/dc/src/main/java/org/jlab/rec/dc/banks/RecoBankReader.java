...
public class RecoBankReader {
     ...
     * @param clAddr address of the cluster
     * @return       the cluster retrieved from the bank
     */
    public FittedCluster getCluster(DataBank clBank, List<FittedHit> hits, int clAddr) {
        if (clBank.rows() == 0) {
        ...
        for (int hID : hIDs) {
            ...
            if (fHit == null) {
                System.out.println("[DCKF] ERROR: hit was not found in list of hits.");
                return null;
            }
            fHits.add(fHit);
        }
     ...
     * @param sAddr    address of the segment
     * @return         the segment retrieved from the bank
     */
    public Segment getSegment(DataBank sBank, List<FittedCluster> clusters, int sAddr) {
        // Assert data correctness
        if (sBank.rows() == 0)     return null;
        if (sAddr >= sBank.rows()) return null;
