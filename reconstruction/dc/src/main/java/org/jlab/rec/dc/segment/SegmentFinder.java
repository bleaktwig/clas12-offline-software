...
public class SegmentFinder {
    ...
    public void get_LayerEfficiencies(Segment seg, DataEvent event, DCGeant4Factory DcDetector) {
        if (seg == null || !event.hasBank("DC::tdc")) return;
        ...
        // First arrays = how many wires off
        ...
        for (int l = 0; l < 6; l++) {
            double z = DcDetector.getWireMidpoint(seg.get_Sector() - 1,
                                                  seg.get_Superlayer() - 1, l, 0).z;
            double trkXMP = seg.get_fittedCluster().get_clusterLineFitSlopeMP() * z
                          + seg.get_fittedCluster().get_clusterLineFitInterceptMP();
            double trkX = seg.get_fittedCluster().get_clusterLineFitSlope() * z
                        + seg.get_fittedCluster().get_clusterLineFitIntercept();

            if (trkX == 0) continue; // Should always get a cluster fit

            int trjWire = trj.getWireOnTrajectory(seg.get_Sector(),
                                                  seg.get_Superlayer(),
                                                  l + 1, trkXMP, DcDetector);
            double x = DcDetector.getWireMidpoint(seg.get_Sector() - 1,
                                                  seg.get_Superlayer() - 1, l, trjWire - 1).x;
            // Math.cos(Math.toRadians(6.)) = 0.9945218953682733
            double cosTrkAngle = 0.9945218953682733
                               * Math.sqrt(1. + seg.get_fittedCluster().get_clusterLineFitSlope()
                                              * seg.get_fittedCluster().get_clusterLineFitSlope());
    ...
    /**
     * Gets a list of segments from the clusters
     * @param allClusters the list of fitted clusters
     * @param missingHits the list of hits that are not in the cluster (should be null for
     *                    time-based tracking)
     * @return            the list of segments obtained
     */
    public List<Segment> get_Segments(List<FittedCluster> allClusters,
                                      DataEvent           event,
                                      DCGeant4Factory     DcDetector,
                                      boolean             runLayersEffs) {

        List<Segment> segList = new ArrayList<Segment>();
        for (FittedCluster fClus : allClusters) {

            if (fClus.size() > Constants.MAXCLUSSIZE) continue;
            if (fClus.get_TrkgStatus() == -1) return segList;

            Segment seg = new Segment(fClus);
            seg.set_fitPlane(DcDetector);

            if (runLayersEffs) this.get_LayerEfficiencies(seg, event, DcDetector);
    ...
    /**
     * Creates and returns a list of time-based fitted clusters.
    ...
    public List<FittedCluster> selectTimeBasedSegments(List<FittedCluster> clusters) {
        ...
        for (int i = 0; i < clusters.size(); i++) {
            ...
            if (AveDoca < AveCelSz*1.2) selTimeBasedSegments.add(clusters.get(i));
        }
