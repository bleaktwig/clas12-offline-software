...
public class TrackCandListFinder {
    ...
    private boolean PassNSuperlayerTracking(List<Cross> crossesInTrk, Track cand) {
        ...
        for (Cross c : crossesInTrk) {
            if (c.isPseudoCross) {
                ...
                if ((c.get_Segment1().get_Status() == 1) ||
                    (c.get_Segment2().get_Status() == 1)) {
                    cand.set_Status(1);
                }
            }
        }
    ...
    private double getHitBasedFitChi2ToCrosses(int sector,
                                               double x1, double y1, double z1,
                                               double x2, double y2, double z2,
                                               double x3, double y3, double z3,
                                               double p, int q,
                                               double x, double y, double z,
                                               double tanThX, double tanThY,
                                               Swim dcSwim) {

        double pz = p / Math.sqrt(tanThX * tanThX + tanThY * tanThY + 1);
        dcSwim.SetSwimParameters(x, y, z, -pz * tanThX, -pz * tanThY, -pz, -q);
    ...
    private double[] getTrackInitFit(int sector,
                                     double x1, double y1, double z1,
                                     double x2, double y2, double z2,
                                     double x3, double y3, double z3,
    ...
    private double calcInitTrkP(double ux, double uy, double uz,
                                double thX, double thY,
                                double theta1, double theta3,
                                double iBdl, double TORSCALE) {
        ...
        return Math.sqrt(pxz * pxz + py * py);
    }

    private int calcInitTrkQ(double theta1, double theta3, double TORSCALE) {
        double deltaTheta = theta3 - theta1;

        // positive charges bend outward for nominal GEMC field configuration
        int q = (int) Math.signum(deltaTheta);
        // flip the charge according to the field scale
    ...
    private void getStraightTrack(Track cand) {
        ...
        for (int i = 0; i < 3; i++) {
            Point3D X = cand.get(i).getCoordsInLab(cand.get(i).get_Point().x(),
                                                   cand.get(i).get_Point().y(),
                                                   cand.get(i).get_Point().z());
            Point3D eX = cand.get(i).getCoordsInLab(cand.get(i).get_PointErr().x(),
                                                    cand.get(i).get_PointErr().y(),
                                                    cand.get(i).get_PointErr().z());
     ...
     * @param stateVec      the track state vector at the last measurement site
     *                          used by the Kalman Filter
     * @param z             the z position in the tilted sector coordinate
     *                          system at the last measurement site
    ...
    public void setTrackPars(Track cand,
        ...
        Cross crossR = new Cross(cand.get(cand.size() - 1).get_Sector(),
                                 cand.get(cand.size() - 1).get_Region(),
                                 -1);
        ...
        dcSwim.SetSwimParameters(stateVec.x(),
                                 stateVec.y(), z,
                                 -pz * stateVec.tanThetaX(),
                                 -pz * stateVec.tanThetaY(),
                                 -pz,
                                 -cand.get_Q());
        ...
        //     traj.set_Trajectory(
        //         trjFind.getStateVecsAlongTrajectory(xOr, yOr, zOr,
        //                                             pxOr/pzOr, pyOr/pzOr,
        //                                             cand.get_P(),cand.get_Q(),
        //                                             getDcDetector));
        //     cand.set_Trajectory(traj.get_Trajectory());
        // }
        ...
        Cross C = new Cross(cand.get(cand.size() - 1).get_Sector(),
        ...
        for (Cross crs : cand) {
            ...
            for (FittedHit h : s1) LR += h._lr;
            for (FittedHit h : s2) LR += h._lr;
        }
     ...
     *
     * Removes the list of tracks that overlap with the track selected based on
     *     best chi2
     */
    ...
    private boolean ListContainsTrack(List<Track> selectedTracks, Track selectedTrk) {
        ...
        for (Track trk : selectedTracks) {
            ...
            if (trk.get_Id() == selectedTrk.get_Id()) isInList = true;
        }
        return isInList;
    }
    ...
    private void getOverlapLists(Track track, List<Track> trkcands, List<Track> list) {
        for (int i = 0; i < trkcands.size(); i++) {
            if ((track.get(0).get_Id() != -1 &&
                 track.get(0).get_Id() == trkcands.get(i).get(0).get_Id()) ||
                (track.get(1).get_Id() != -1 &&
                 track.get(1).get_Id() == trkcands.get(i).get(1).get_Id()) ||
                (track.get(2).get_Id() != -1 &&
                 track.get(2).get_Id() == trkcands.get(i).get(2).get_Id())) {
    ...
    public void matchHits(List<StateVec> stateVecAtPlanesList, Track trk,
        ...
        if (stateVecAtPlanesList == null) return;
        ...
        double[] ToFirstMeas =
            dcSwim.SwimToPlaneTiltSecSys(trk.get(0).get_Sector(),
    ...
    private double calcCurvSign(Track cand) {
        ...
        // Find the intersection of the lines joining the innermost to middle
        //     and middle to outermost point
