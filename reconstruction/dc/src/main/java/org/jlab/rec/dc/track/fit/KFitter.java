...
// %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            ...
            if (i > 1) {
                ...
                if (this.chi2kf < newChisq) {
                    if (this.finalStateVec != null &&
                        Math.abs(sv.trackTraj.get(svzLength-1).Q -this.finalStateVec.Q)  < 5.e-4 &&
    ...
    private void filter(int k) {
        if (sv.trackTraj.get(k) != null &&
            ...
            double[][] HTGH =
                new double[][]{{H[0] * H[0] / V, H[0] * H[1] / V, 0, 0, 0},
            ...
            // auxiliary variable:
            double mvMeasAux = mv.measurements.get(k).x - h;

            double c2 = ((mvMeasAux) * (mvMeasAux) / V);

            double x_filt  = sv.trackTraj.get(k).x  + K[0] * (mvMeasAux);
            double y_filt  = sv.trackTraj.get(k).y  + K[1] * (mvMeasAux);
            double tx_filt = sv.trackTraj.get(k).tx + K[2] * (mvMeasAux);
            double ty_filt = sv.trackTraj.get(k).ty + K[3] * (mvMeasAux);
            double Q_filt  = sv.trackTraj.get(k).Q  + K[4] * (mvMeasAux);
    ...
    private void calcFinalChisq(int sector) {
        ...
        chi2 += (mv.measurements.get(0).x - h0) *
                (mv.measurements.get(0).x - h0) /
        ...
        for (int k1 = 0; k1 < k; k1++) {
            ...
            double h = mv.h(new double[]{sv.trackTraj.get(k1 + 1).x,
            ...
            svc.setProjector(h);
            kfStateVecsAlongTrajectory.add(svc);
            chi2 += (mv.measurements.get(k1 + 1).x - h) *
                    (mv.measurements.get(k1 + 1).x - h) / V;
        }
    }
