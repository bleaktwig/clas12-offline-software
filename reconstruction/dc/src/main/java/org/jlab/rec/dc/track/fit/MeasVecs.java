...
public class MeasVecs {
    ...
    public void setMeasVecs(Track trkcand, DCGeant4Factory DcDetector) {
        ...
        FittedHit hitOnTrk;
        // Loops over the regions (1 to 3) and the superlayers in a region (1 to 2) to obtain the
        // hits on track
        for (int c = 0; c < 3; c++) {
            for (int s = 0; s < 2; s++) {
                for (int h = 0; h < trkcand.get(c).get(s).size(); h++) {
                    if (trkcand.get(c).get(s).get(h).get_Id() == -1) continue;
                    ...
                    double sl1 =
                            trkcand.get(c).get(s).get_fittedCluster().get_clusterLineFitSlope();
                    double it1 =
                    ...
                    double err_sl1 =
                            trkcand.get(c).get(s).get_fittedCluster().get_clusterLineFitSlopeErr();
                    double err_it1 =
                            trkcand.get(c).get(s).get_fittedCluster().get_clusterLineFitInterceptErr();
                    double err_cov1 =
                            trkcand.get(c).get(s).get_fittedCluster().get_clusterLineFitSlIntCov();
                    ...
                    if (Math.abs(trkcand.get(c).get(s).get(h).get_Residual()) < 1) hOTS.add(hot);
                }
            }
        }
        // Sort the collection in order of increasing Z value (i.e. going downstream from the target)
        ...
        for (int i = 0; i < hOTS.size(); i++) {
            if (i > 0) {
                if (Math.abs(hOTS.get(i - 1)._Z - hOTS.get(i)._Z) < 0.01) {
                    hOTS.get(i - 1)._X = (hOTS.get(i - 1)._X
    ...
    void setMeasVecsFromHB(Track trk, DCGeant4Factory DcDetector) {
        ...
        for (int s = 0; s < trk.get_ListOfHBSegments().size(); s++) {
            for (int h = 0; h < trk.get_ListOfHBSegments().get(s).size(); h++) {
                ...
                double sl1 = trk.get_ListOfHBSegments().get(s)
                                .get_fittedCluster().get_clusterLineFitSlope();
                double it1 = trk.get_ListOfHBSegments().get(s)
                                .get_fittedCluster().get_clusterLineFitIntercept();
                ...
                double err_sl1 = trk.get_ListOfHBSegments().get(s)
                                    .get_fittedCluster().get_clusterLineFitSlopeErr();

                double err_it1 = trk.get_ListOfHBSegments().get(s)
                                    .get_fittedCluster().get_clusterLineFitInterceptErr();

                double err_cov1 = trk.get_ListOfHBSegments().get(s)
                                     .get_fittedCluster().get_clusterLineFitSlIntCov();
