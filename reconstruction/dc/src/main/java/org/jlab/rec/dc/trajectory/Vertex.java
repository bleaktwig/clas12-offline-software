...
public class Vertex {
    ...
    /**
     * Estimates the vertex at the raster radius estimated from MC.
    ...
    public double vertexEstimator(DataEvent event) {
        ...
        if (event.hasBank("MC::Particle")) {
            ...
            // Analysis done in cm, gemc vtx units = mm
            double val = Math.sqrt(bank.getFloat("vx", 0) * bank.getFloat("vx", 0)
                                 + bank.getFloat("vy", 0) * bank.getFloat("vy", 0))/10.;
    ...
    /**
     * Resets a track at the estimated raster radius.
     * @param event   HipoDataEvent
    ...
    public void resetTrackAtRasterRadius(DataEvent event, Track thecand) {
        ...
        if (Math.sqrt(x0*x0 + y0*y0) > r) return;
    ...
    /**
     * Creates and returns a vertex reconstructed at the distance of closest approach to the beam
     * axis.
     * @param x      track x
     * @param y      track y
     * @param z      track z
    ...
    public double[] VertexParams(double x, double y, double z, double px, double py, double pz,
        ...
        if (Bfield < 0.0000001) {
            ...
            double phi_dca;
            if (Q > 0) phi_dca = Math.atan2( (xc-xb),  (-yc+yb));
            else       phi_dca = Math.atan2(-(xc-xb), -(-yc+yb));
