...
public class DCEngine extends ReconstructionEngine {
    ...
    public void setStartTimeOption() {
        ...
        if (useSTTConf != null) {
            System.out.println("[" + this.getName() + "] run with start time in tracking config "
                             + "chosen based on yaml =" + useSTTConf);
            Constants.setUSETSTART(Boolean.valueOf(useSTTConf));
        }
        else useSTTConf = System.getenv("USESTT");

        if (useSTTConf != null) {
            System.out.println("[" + this.getName() + "] run with start time in tracking config "
                             + "chosen based on env =" + useSTTConf);
            Constants.setUSETSTART(Boolean.valueOf(useSTTConf));
        }
        else {
             System.out.println("[" + this.getName() + "] run with start time in tracking config "
                              + "chosen based on default =" + Constants.isUSETSTART());
        }
        ...
        if (wireDistortionsFlag != null) {
            System.out.println("[" + this.getName() + "] run with wire distortions in tracking "
                             + "config chosen based on yaml =" + wireDistortionsFlag);
            if (Boolean.valueOf(wireDistortionsFlag) == true) Constants.setWIREDIST(1.0);
            else                                              Constants.setWIREDIST(0);
        }
        else wireDistortionsFlag = System.getenv("USEWIREDIST");

        if (wireDistortionsFlag != null) {
            System.out.println("[" + this.getName() + "] run with wire distortions in tracking "
                             + "config chosen based on env =" + wireDistortionsFlag);
            if (Boolean.valueOf(wireDistortionsFlag) == true) Constants.setWIREDIST(1.0);
            else                                              Constants.setWIREDIST(0);
        }
        else {
             System.out.println("[" + this.getName() + "] run with default setting for wire "
                              + "distortions in tracking (off in MC, on in data)");
        }
    }
    ...
    public void LoadTables() {
        ...
        if (geomDBVar != null) {
            System.out.println("[" + this.getName() + "] run with geometry variation based on "
                             + "yaml =" + geomDBVar);
        }
        else geomDBVar = System.getenv("GEOMDBVAR");

        if (geomDBVar != null) {
            System.out.println("[" + this.getName() + "] run with geometry variation chosen based "
                             + " on env =" + geomDBVar);
        }
        else System.out.println("[" + this.getName() + "] run with default geometry");
        ...
        for (int l = 0; l < 6; l++) {
            ...
            System.out.println("****************** WPDIST READ ********* FROM " + geomDBVar
                             + "**** VARIATION ****** "
                             + provider.getDouble("/geometry/dc/superlayer/wpdist", l));
        }
        ...
        if (ccDBVar != null) {
            System.out.println("[" + this.getName() + "] run with constants variation based on "
                             + " yaml =" + ccDBVar);
        }
        else ccDBVar = System.getenv("CCDBVAR");

        if (ccDBVar != null) {
            System.out.println("[" + this.getName() + "] run with constants variation chosen based "
                             + " on env =" + ccDBVar);
        }
        else System.out.println("["+this.getName()+"] run with default constants");
