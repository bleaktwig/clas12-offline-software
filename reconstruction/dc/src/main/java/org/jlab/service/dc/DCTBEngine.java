...
public class DCTBEngine extends DCEngine {
    ...
    public boolean processDataEvent(DataEvent event) {
        ...
        clusters = clusFinder.FindTimeBasedClusters(hits, cf, ct,
            super.getConstantsManager().getConstants(newRun,
                "/calibration/dc/time_to_distance/time2dist"), dcDetector, tde);
        ...
        if(trkcands.size() > 0) {
            ...
            for(Track trk: trkcands) {
                ...
                trk.calcTrajectory(trkId, dcSwim,
                                   trk.get_Vtx0().x(),    trk.get_Vtx0().y(),
                                   trk.get_Vtx0().z(),    trk.get_pAtOrig().x(),
                                   trk.get_pAtOrig().y(), trk.get_pAtOrig().z(),
                                   trk.get_Q(), ftofDetector, tSurf);
