...
public class TrackDictionaryValidation {
    ...
    public void analyzeHistos() {
        ...
        System.out.println("Positive particles found/missed: "
                + this.dataGroups.getItem(2).getH2F("hi_ptheta_pos_found").integral() + "/" +
                + this.dataGroups.getItem(2).getH2F("hi_ptheta_pos_missing").integral());
        System.out.println("Negative particles found/missed: "
                + this.dataGroups.getItem(2).getH2F("hi_ptheta_neg_found").integral() + "/" +
                + this.dataGroups.getItem(2).getH2F("hi_ptheta_neg_missing").integral());
    }

    public void createDictionary(String inputFileName, String dictName , int pidSelect,
            int chargeSelect, double thrs, int duplicates) {
        ...
        while(reader.hasEvent() == true) {
            ...
            nevent++;
            if (nevent%10000 == 0) System.out.println("Analyzed " + nevent + " events, found "
                    + newDictionary.size() + " roads");
            ...
            if (event.hasBank("RUN::config")) runConfig = event.getBank("RUN::config");
            if (event.hasBank("REC::Particle")) recParticle = event.getBank("REC::Particle");
            if (event.hasBank("REC::Scintillator")) recScintillator = event.getBank("REC::Scintillator");
            if (event.hasBank("REC::Calorimeter")) recCalorimeter = event.getBank("REC::Calorimeter");
            if (event.hasBank("REC::Cherenkov")) recCherenkov = event.getBank("REC::Cherenkov");
            if (event.hasBank("REC::Track")) recTrack = event.getBank("REC::Track");
            if (event.hasBank("TimeBasedTrkg::TBTracks")) tbtTrack = event.getBank("TimeBasedTrkg::TBTracks");
            if (event.hasBank("TimeBasedTrkg::TBHits")) tbtHits = event.getBank("TimeBasedTrkg::TBHits");
            if (event.hasBank("ECAL::clusters")) ecalCluster = event.getBank("ECAL::clusters");
            if (event.hasBank("HTCC::rec")) htccRec = event.getBank("HTCC::rec");
            ...
            if(recParticle != null && recTrack != null && tbtTrack != null && tbtHits != null) {
                for (int i = 0; i < recTrack.rows(); i++) {
                    ...
                    if (detector == DetectorType.DC.getDetectorId()) {
                        ...
                        // Use only roads with momentum above the selected value
                        if (part.p() < thrs) continue;
                                    ...
                                    if (recCherenkov.getShort("pindex", j) == pindex) {
                                        ...
                                        if (detectorCheren == DetectorType.HTCC.getDetectorId()) {
                                            ...
                                            for (int iPMT = 0; iPMT < htccPMTs.size(); iPMT++) {
                                                ...
                                                if (found) htccPMTsMatched.add(htccPMTs.get(iPMT));
                                            }
                                            if (htccPMTsMatched.size() != nhits) {
                                                System.out.println("Mismatch in HTCC cluster size "
                                                        + runConfig.getInt("event",0) + " " + nhits
                                                        + "/" + htccPMTsMatched.size() + "/"
                                                        + htccPMTs.size() + " " + thetaCC + " "
                                                        + phiCC + " " + ((phiCC+30)%60-30));
                                            }
    ...
    public ArrayList<int[]> htccPMT(double th, double ph) {
        ...
        int sector=0;
        if      (ph >  30.0 && ph <=  90.0) sector = 2;
        else if (ph >  90.0 && ph <= 150.0) sector = 3;
        else if (ph > 150.0 && ph <= 210.0) sector = 4;
        else if (ph > 210.0 && ph <= 270.0) sector = 5;
        else if (ph > 270.0 && ph <= 330.0) sector = 6;
        else if (ph <= 30.0 || ph >  330.0) sector = 1;
