...
// === GET CROSSES =============================================================
        ...
        System.out.println("[DCKF] " + dataeventCounter + ".2: banks loaded.");

        // pull the crosses from the bank.
        int nCrosses = crossesBank.rows();
        if (nCrosses == 0) return true;
...
// === CREATE AND RUN TrackCandListFinder ======================================
    ...
    // TODO: Move these get methods to RecoBankReader
    // TODO: Write a manager method to avoid creating referenced objects
    //       multiple times.
    private Cross getCross(DataBank crBank, DataBank sBank,
    ...
    private Segment getSegment(DataBank sBank, DataBank clBank,
        ...
        /* WARNING: SEGMENT DATA IN BANK CURRENTLY UNUSED:
    ...
    private FittedCluster getCluster(DataBank clBank, DataBank hBank, int clAddr) {
        if (clBank.rows() == 0) {
            System.out.println("[DCKF] ERROR: clusters bank is empty.");
            return null;
        }
        ...
        /* WARNING: CLUSTER DATA IN BANK CURRENTLY UNUSED:
    ...
    private FittedHit getHit(DataBank bank, int addr) {
        if (bank.rows() == 0) {
            System.out.println("[DCKF] ERROR: hits bank is empty.");
            return null;
        }
